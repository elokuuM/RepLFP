...
sal_path = "../test_out/Wlpce_graycrf"
filled_img_pseudo_path = "../dataset/train/filled_transformer_crf_gt" 
filled_mask_pseudo_path = '../dataset/train/filled_transformer_crf_mask'
...
for file in file_list:
    ...
    if (os.path.isfile(os.path.join(sal_path, file))):
        edge_map_orig = cv2.imread(os.path.join(sal_path, file), 0).astype(np.float32) #0 gray
        edge_map_orig[edge_map_orig < 240] = 0     # leave high-confidence foreground   
        edge_map_orig[edge_map_orig == 255] = 254  # empty position 255
        # plt.imshow(edge_map_orig)
        # plt.show()
        edge_map = edge_map_orig.copy()
        print('imageshape: ', edge_map.shape)
...
for file in file_list:
    ...
    if (os.path.isfile(os.path.join(sal_path, file))):
        ...
        cv2.imwrite(os.path.join(3, file), background_map)  # background map，it will be replaced by mask map
