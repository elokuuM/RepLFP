...
class LocalSaliencyCoherence(torch.nn.Module):
    ...
    def forward(
        ...
        dilation = kernels_desc[0]['dilation']
        ...
        y_hat_unfolded = self._unfold(y_hat_softmax, kernels_radius, dilation)
    ...
    def _create_kernels(
        ...
        for i, desc in enumerate(kernels_desc):
            ...
            kernel = weight * LocalSaliencyCoherence._create_kernels_from_features(features, kernels_radius, dilation)
            kernels = kernel if kernels is None else kernel + kernels
        return kernels

    @staticmethod
    def _create_kernels_from_features(features, radius, dilation):
        assert features.dim() == 4, 'Features must be a NCHW batch'
        N, C, H, W = features.shape
        kernels = LocalSaliencyCoherence._unfold(features, radius, dilation)
    ...
    @staticmethod
    def _unfold(img, radius, dilation):
        assert img.dim() == 4, 'Unfolding requires NCHW batch'
        N, C, H, W = img.shape
        diameter = 2 * radius + 1
        return F.unfold(img, diameter, dilation, radius*dilation).view(N, C, diameter, diameter, H, W)  # -> (N, C*K_h*k_w, 滑了多少次).view(N,C,K_h,K_w,H,W)
