...
import copy
from models.backbone.MobileOne_Repmlp import MobileOne, MobileOneBlock
...
class RepAsppBlock(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super(RepAsppBlock, self).__init__()
        ...
        self.channelconv = MobileOneBlock(in_channels=self.in_channels,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.inference_mode:
            ...
            out = [x]
            for idx in range(len(self.dilation)):
                out.append(self.activation(self.reparam_conv[idx](out[-1])))
                
            out = self.invertconv(sum(out[1:])) + short_cut
        ...
        for idx in range(len(self.dilation)):
            ...
            if self.rbr_skip is not None:
                identity_out = self.rbr_skip[idx](out[-1])
    
            # Scale branch output
            scale_out = 0
            if self.rbr_scale is not None:
                scale_out = self.rbr_scale[idx](out[-1])
    
            # Other branches
            conv_out = 0
            for ix in range(self.num_conv_branches):
                conv_out += self.rbr_conv[(idx*self.num_conv_branches)+ix](out[-1])

            out.append(self.activation(identity_out + scale_out + conv_out))            
...
class RepAsppdecoder(nn.Module):         #enc_channels=[48, 48, 128, 256]  #(64, 96, 192, 512)
    def __init__(self, inference_mode = False, enc_channels = [64, 96, 192, 512], expand_ratio = 0.5):
        super().__init__()
        self.inference_mode = inference_mode
        
        self.deoder1 = RepAsppBlock(enc_channels[-1], enc_channels[-2],
                                    dilation = [1, 2, 5],           #[1, 2, 3, 4]     [1, 2]
                                    expand_ratio = expand_ratio,
                                    inference_mode = self.inference_mode)
                       
        self.deoder2 = RepAsppBlock(enc_channels[-2], enc_channels[-3],
                                    dilation = [1, 2, 5, 9],           #[1, 2, 4, 8]     [1, 2, 3, 4]
                                    expand_ratio = expand_ratio,
                                    inference_mode = self.inference_mode)
                       
        self.deoder3 = RepAsppBlock(enc_channels[-3], enc_channels[-4],
                                    dilation = [1, 2, 5, 9],           #[1, 4, 8, 16]    [1, 2, 4, 8]
                                    expand_ratio = expand_ratio,
                                    inference_mode = self.inference_mode)
                       
        self.deoder4 = RepAsppBlock(enc_channels[-4], 32,
                                    dilation = [1, 2, 5, 9, 17],           #[1, 4, 8, 16]    [1, 4, 8, 16]
                                    expand_ratio = expand_ratio,
                                    inference_mode = self.inference_mode)
        
        self.convout = nn.Sequential(nn.Conv2d(32, 16, 3, 1, 1),nn.BatchNorm2d(16), nn.ReLU(inplace=True),nn.Conv2d(16, 1, 1, 1, padding=0))
                                    
        self.side_out1  = nn.Conv2d(enc_channels[-2], 1, kernel_size=1, stride=1, padding=0)
        self.side_out2  = nn.Conv2d(enc_channels[-3], 1, kernel_size=1, stride=1, padding=0)
        self.side_out3  = nn.Conv2d(enc_channels[-4], 1, kernel_size=1, stride=1, padding=0)
...
class mboUrepaspp_s0(nn.Module):
    def __init__(self, cfg, **kwargs):
        super().__init__()
        if cfg.mode == 'train':
            self.encoder = MobileOne(inference_mode = False, width_multipliers = (0.75, 1.0, 1.0, 2.0), num_conv_branches = 4)
            if cfg.pretrain:
                print('load pretrained model !-----')
                print('unexpected layername:', \
                self.encoder.load_state_dict(torch.load('./models/mobileone_s0_unfused.pth.tar', map_location='cpu'), strict=False))
                print('load pretrained finish ! -----')
            self.decoder = RepAsppdecoder(inference_mode = False, enc_channels = [48, 48, 128, 256], expand_ratio = 0.5)
        elif cfg.mode == 'test':
            self.encoder = MobileOne(inference_mode = True, width_multipliers = (0.75, 1.0, 1.0, 2.0), num_conv_branches = 4)
            self.decoder = RepAsppdecoder(inference_mode = True, enc_channels = [48, 48, 128, 256], expand_ratio = 0.5)
        else:
            print('mode error!')
    def forward(self, x):
        x = self.encoder(x)
...
class mboUrepaspp_s1(nn.Module):
    def __init__(self, cfg, **kwargs):
        super().__init__()
        if cfg.mode == 'train':
            ...
            if cfg.pretrain:
                ...
                print('load pretrained finish ! -----')
            self.decoder = RepAsppdecoder(inference_mode = False, enc_channels = [64, 96, 192, 512])
        elif cfg.mode == 'test':
            self.encoder = MobileOne(inference_mode = True, width_multipliers = (1.5, 1.5, 2.0, 2.5))
            self.decoder = RepAsppdecoder(inference_mode = True, enc_channels = [64, 96, 192, 512])
        else:
            print('mode error!')
    def forward(self, x):
        x = self.encoder(x)
