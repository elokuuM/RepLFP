...
class UniRepLKNetBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if kernel_size == 0:
            ...
            self.dwconv = DilatedReparamBlock(dim, kernel_size, deploy=self.deploy,
            ...
            self.dwconv = get_conv2d(dim, dim, kernel_size=kernel_size, stride=1, padding=kernel_size // 2,
                                     dilation=1, groups=dim, bias=self.deploy,
                                     attempt_use_lk_impl=attempt_use_lk_impl)

        if self.deploy or kernel_size == 0:
        ...
        self.act = nn.Sequential(
            nn.GELU(),
            GRNwithNHWC(ffn_dim, use_bias=not self.deploy))
        if self.deploy:
        ...
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones(dim),
                                  requires_grad=True) if (not self.deploy) and layer_scale_init_value is not None \
...
if __name__ == '__main__':
    ...
    model = UniRepLKNetBackbone(init_cfg={'checkpoint':None})
    ...
    for idx in range(len(out1)):
        print(out1[idx].shape)
    print(torch.allclose(out[-1], out1[-1]))
