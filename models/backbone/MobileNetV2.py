...
class MobileNetV2(nn.Module):
    def __init__(self, pretrained=None, num_classes=1000, width_mult=1.0):
        ...
        if pretrained is not None:
            print(self.load_state_dict(torch.load('./models/mobilenet_v2-b0353104.pth', map_location='cpu'), strict=False))

    def forward(self, x): 
        res = []
        for idx, m in enumerate(self.features):
            x = m(x)
            if idx in [1,3,6,13,16]:
                res.append(x)
        return res


def mobilenetv2(pretrained=None):
    if pretrained:
        print("loading pretrained model")
        return MobileNetV2(pretrained = True)#pretrained="pretrained/mobilenet_v2-b0353104.pth")
    else:
        return MobileNetV2()
