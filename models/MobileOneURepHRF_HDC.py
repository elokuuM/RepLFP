...
from models.UNetFormer import Decoder
...
class RepAsppBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.inference_mode:
            ...
            out = x
            for idx in range(len(self.dilation)):
                out= self.activation(self.reparam_conv[idx](out))
        ...
        for idx in range(len(self.dilation)):
            ...
            if self.rbr_skip is not None:
                identity_out = self.rbr_skip[idx](out)
    
            # Scale branch output
            scale_out = 0
            if self.rbr_scale is not None:
                scale_out = self.rbr_scale[idx](out)
    
            # Other branches
            conv_out = 0
            for ix in range(self.num_conv_branches):
                conv_out += self.rbr_conv[(idx*self.num_conv_branches)+ix](out)

            out = self.activation(identity_out + scale_out + conv_out)          
