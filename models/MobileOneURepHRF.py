...
class RepHRFdecoder(nn.Module):         #enc_channels=[48, 48, 128, 256]  #(64, 96, 192, 512)
    def __init__(self, inference_mode = False, enc_channels = [64, 96, 192, 512], expand_ratio = 0.5):
        super().__init__()
        self.inference_mode = inference_mode
        
        self.deoder1 = RepHRFBlock(enc_channels[-1], enc_channels[-2],
                                   dilation = [1, 2, 3, 4],  #[1, 2, 3, 4, 5]     [1, 2]
                                   expand_ratio = expand_ratio,
                                   inference_mode = self.inference_mode)
                       
        self.deoder2 = RepHRFBlock(enc_channels[-2], enc_channels[-3],
                                   dilation = [1, 2, 4, 8],  #[1, 2, 4, 8, 12]     [1, 2, 3, 4]
                                   expand_ratio = expand_ratio,
                                   inference_mode = self.inference_mode)
                       
        self.deoder3 = RepHRFBlock(enc_channels[-3], enc_channels[-4],
                                   dilation = [1, 4, 8, 16],  #[1, 4, 8, 16, 24]    [1, 2, 4, 8]
                                   expand_ratio = expand_ratio,
                                   inference_mode = self.inference_mode)
                       
        self.deoder4 = RepHRFBlock(enc_channels[-4], 32,
                                   dilation = [1, 4, 8, 16],  #[1, 4, 8, 16, 32]    [1, 4, 8, 16]
                                   expand_ratio = expand_ratio,
                                   inference_mode = self.inference_mode)
    ...
    def forward(self, x):
        ...
        if self.inference_mode:
            out = F.interpolate(self.deoder1(out4), scale_factor = (2,2), mode='bilinear', align_corners=False)  + out3 # 28
            out = F.interpolate(self.deoder2(out), scale_factor =(2,2), mode='bilinear', align_corners=False)   + out2 # 56
            out = F.interpolate(self.deoder3(out), scale_factor =(2,2), mode='bilinear', align_corners=False)   + out1 # 112
...
class MobileOneURepHRF_s0(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        if cfg.mode == 'train':
            ...
            self.decoder = RepHRFdecoder(inference_mode = False, enc_channels = [48, 48, 128, 256], expand_ratio = 0.5)
        elif cfg.mode == 'test':
            self.encoder = MobileOne(inference_mode = True, width_multipliers = (0.75, 1.0, 1.0, 2.0), num_conv_branches = 4)
            self.decoder = RepHRFdecoder(inference_mode = True, enc_channels = [48, 48, 128, 256], expand_ratio = 0.5)
        else:
            print('mode error!')
    def forward(self, x):
        x = self.encoder(x)
...
#                 print('load pretrained finish ! -----')
#             self.decoder = RepHRFdecoder(inference_mode = False, enc_channels = [64, 96, 192, 512])
#         elif cfg.mode == 'test':
#             self.encoder = MobileOne(inference_mode = True, width_multipliers = (1.5, 1.5, 2.0, 2.5))
#             self.decoder = RepHRFdecoder(inference_mode = True, enc_channels = [64, 96, 192, 512])
#         else:
#             print('mode error!')
#     def forward(self, x):
#         x = self.encoder(x)
